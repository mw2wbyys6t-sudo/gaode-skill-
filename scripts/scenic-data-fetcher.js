...
async function fetchAround(lng, lat, apiKey, options = {}) {
  ...
  console.log(`[${searchLabel}] 中心: (${lng.toFixed(4)}, ${lat.toFixed(4)}), 半径: ${radius}m, 类型: ${types}`);
  ...
  try {
    ...
    if (data.status !== '1' && data.status !== 1) {
      console.warn(`[${searchLabel}] 高德返回错误: ${data.info || '未知'}`);
      return [];
    }

    const rawPois = data.pois || [];
    console.log(`[${searchLabel}] 获取到 ${rawPois.length} 条 POI 数据`);

    return rawPois.map(enrichPoi);
  } catch (err) {
    console.warn(`[${searchLabel}] 请求失败: ${err.message}`);
    return [];
  }
}
