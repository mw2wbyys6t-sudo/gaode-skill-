...
description: 高德地图综合服务，支持 POI 搜索、路径规划、旅游规划、美食 POI 发现与分类、周边搜索和热力图数据可视化。高级功能包括：4 阶段智能旅游规划流水线（LLM 意图解析→多策略 POI 搜索→TSP 路线优化→交互式地图生成）、AI 旅伴聊天（小次）、Edge TTS 语音导览。v3.0 新增餐饮选址分析：多半径竞争密度扫描、商圈画像、100 分制 5 维选址评分、多商圈对比、双格式选址报告（HTML 交互式报告 + Word 专业文档含雷达图）。支持中国大陆（含港澳台）和非中国大陆（不含港澳台）Web API 场景。
version: 3.0.0
metadata:
  openclaw:
    requires:
...
高德地图综合服务向开发者提供完整的地图数据服务，包括地点搜索、路径规划、旅游规划、餐饮选址分析和数据可视化等功能。
...
- 餐饮选址与开店咨询（如"在春熙路开火锅店怎么样"、"奶茶店选址"、"哪里适合开餐厅"、"商圈分析"）
- 包含"选址"、"开店"、"开餐厅"、"找铺面"、"铺位"、"商圈分析"、"竞争分析"、"人流量"、"月租"、"餐饮创业"等关键词
...
- **场景十二**：餐饮选址分析（智能选址评分、竞争密度扫描、商圈画像、选址报告生成）
...
> **注意**：此场景基于已移除的 `gaode_skill.py`，不再可用。请使用以下替代方案：
> - POI 搜索 → **场景四**（JavaScript `scripts/scenic-data-fetcher.js`）
> - 路径规划 → **场景五**（JavaScript `scripts/route-planning.js`）
...
用户提到以下关键词时触发选址分析：选址、开店、开餐厅、找铺面、铺位、商圈分析、竞争分析、人流量、月租、餐饮创业、加盟店、适合开、能不能开、哪里适合、开火锅店、开奶茶店。

### 工作流程

1. **意图解析**：从用户输入中提取餐饮类型、目标城市、候选商圈、月租预算、店铺类型
2. **多商圈竞争扫描**：对每个候选区域执行 150m/500m/1km/3km 四层同心圆 POI 扫描
3. **商圈画像生成**：分析 POI 构成、交通配套、互补业态，生成商圈画像
4. **选址评分**：100 分制评分（战略 20 + 竞争 20 + 销售 20 + 配套 20 + 立地 20）
5. **报告生成**：生成包含雷达图、竞争地图、对比卡片的交互式 HTML 报告

### API 端点

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/site-analysis?input=在成都春熙路开火锅店&city=成都` | 端到端选址分析 |
| POST | `/api/site-compare` | 多商圈对比（body: `{areas, restaurant_type, city}`） |
| POST | `/api/site-chat` | 选址顾问对话（body: `{sessionId, message, city}`） |

### 选址评分模型

| 维度 | 分值 | 说明 |
|------|------|------|
| 战略因素 | 20 分 | 城市等级 + 商圈类型 + 发展趋势 |
| 竞争因素 | 20 分 | 核心圈密度（反直觉：竞争越多 = 成熟商圈 = 加分）+ 同品类占比 |
| 销售潜力 | 20 分 | 人流代理指标 + 捕获率估算 |
| 服务配套 | 20 分 | 互补业态评分 + 交通可达性 |
| 立地条件 | 20 分 | 区域类型 + 街道可达性 + 物业可行性 |

**决策阈值**：>=80 优秀（立即行动）、>=60 良好（可以进驻）、>=40 及格（谨慎考虑）、<40 不推荐（建议放弃）

### 示例交互

**用户**：我想在成都春熙路开一家火锅店，月租2万以内

**系统响应**：
- 扫描春熙路 150m/500m/1km/3km 四层竞争数据
- 输出评分：82 分（良好）
- 展示竞争分布：150m 内 87 家餐饮（成熟商圈加分）
- 菜系细分：火锅 12 家（占比 14%，有差异化空间）
- LLM 综合建议："春熙路属于成熟商圈，竞争密度高但人流充足..."

### 图片生成

选址分析完成后，可使用 ImageGen 工具生成：
- 商圈鸟瞰概念图（作为报告封面）
- 评分摘要信息图（用于社交分享）
...
| 模块 | 文件 | 功能 |
|------|------|------|
| 意图解析 | `scripts/site-intent-parser.js` | LLM + 正则双模式解析 |
| 竞争扫描 | `scripts/competition-scanner.js` | 四层同心圆 POI 扫描 |
| 商圈画像 | `scripts/area-profiler.js` | POI 构成 + 交通 + 互补业态 |
| 评分引擎 | `scripts/site-analyzer.js` | 100 分制 5 维度评分 |
| 流水线 | `scripts/site-pipeline.js` | 5 阶段编排 |
| 报告模板 | `templates/site-report.html` | 交互式 HTML 报告 |
...
```json
{
  "amapWebServiceKey": "your_amap_webservice_key_here",
  "amapOverseasWebServiceKey": "your_overseas_amap_webservice_key_here",
  "appname": "gaode-map-lbs"
}
```
